/*
  # Complete System Setup and Fixes
...
DROP POLICY IF EXISTS "Users can manage their own study plans" ON study_plans;
...
CREATE TABLE IF NOT EXISTS study_plan_sessions (
  ...
  session_type text DEFAULT 'study' CHECK (session_type IN ('study', 'review', 'quiz', 'break')),
  subject text,
...
DROP POLICY IF EXISTS "Users can manage their own plan sessions" ON study_plan_sessions;
CREATE POLICY "Users can manage their own plan sessions"
  ON study_plan_sessions FOR ALL
  TO authenticated
  USING (uid() = user_id)
  WITH CHECK (uid() = user_id);
...
CREATE INDEX IF NOT EXISTS idx_study_plan_sessions_date ON study_plan_sessions(session_date);